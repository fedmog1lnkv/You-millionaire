...
define config.name = _("Dissociation")
...
define gui.about = _p("""
На стадии разработки мы заложили фундаментальные принципы игры

Мы можем выбрать от лица кого, мы будем следить за историей,
каждый персонаж уникальный, и не всегда знаешь, что у него на уме

Игру нужно перепроходить несколько раз, у нее закольцованный
сюжет, который каждый раз возврщает героя к тому же, от чего
он начал

Только исследовав многих персонажей, узнав о их мыслях, решениях
возможно приблизиться к разгадке истории

В нашей игре концовки не имеют смысла, важнее процесс, и почему
это истинна предстоит узнать самому игроку.

Это не просто поверхностная игра, в ней заложен глубокий сюжет.

То что лежит на поверхности, как кажется игроку, имеет совсем
другой смысл, чем кажется по началу игры.
...
define build.name = "Youmillionaire"
...
define config.save_directory = "Youmillionaire-1662617643"
